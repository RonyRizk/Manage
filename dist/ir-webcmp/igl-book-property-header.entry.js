import { r as registerInstance, a as createEvent, h, e as Host } from './index-c229806f.js';
...
const IglBookPropertyHeader = class {
  ...
  handleButtonClicked() {
    if (this.isEventType('SPLIT_BOOKING') && Object.keys(this.bookedByInfoData).length <= 1) {
      ...
      this.toast.emit({
        type: 'error',
        title: `${locales.entries.Lcz_CheckInDateShouldBeMAx} ${hooks(new Date(this.bookedByInfoData.to_date || this.defaultDaterange.to_date)).format('ddd, DD MMM YYYY')} `,
        description: '',
        position: 'top-right',
      });
    }
